      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {caseStudies.map((study, index) => (
            <div key={index} className="bg-card rounded-2xl overflow-hidden border border-border card-hover" data-testid={`case-study-card-${index}`}>
              <img 
                src={study.image} 
                alt={study.alt} 
              ...
              <div className="p-6">
                <div className="flex items-center justify-between mb-4">
                  <span className="bg-primary/10 text-primary px-3 py-1 rounded-full text-sm font-medium" data-testid={`case-study-category-${index}`}>
                    {study.category}
                  </span>
                  <span className="text-sm text-muted-foreground" data-testid={`case-study-date-${index}`}>
                    {study.date}
                  </span>
                </div>
                <h3 className="text-xl font-semibold mb-3" data-testid={`case-study-title-${index}`}>{study.title}</h3>
                <p className="text-muted-foreground mb-4" data-testid={`case-study-description-${index}`}>
                  {study.description}
                </p>
                ...
                <button className="text-primary font-medium hover:underline" data-testid={`case-study-link-${index}`}>
                  Read Full Story →
                </button>
              </div>
            </div>
